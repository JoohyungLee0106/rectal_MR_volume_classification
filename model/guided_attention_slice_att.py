...
class Attention(nn.Module):
    def __init__(self, resnet_type=34, encoder_dim_type='2d', att_type=[False]*4, stride_D1=[1, 2, 2, 2],
        ...
        self.att_loc = []
        self.global_pool = nn.AdaptiveAvgPool3d((7, 1, 1))
    ...
    def forward(self, x):
        ...
        cl = self.global_pool(cl)
        cl = (cl.flatten(2)).permute(0, 2, 1)
        cl = self.encoder.fc(cl)
        cl = cl.squeeze(2)


        # if self.slice_att == 'log':
        #     w = -(torch.log(1.0-torch.sigmoid(cl)))
        #     cl2 = torch.mean(cl*w, dim=1, keepdim=False)
        # elif self.slice_att == 'l2':
        #     w = (1.0-torch.sigmoid(cl))**2
        #     cl2 = torch.mean(cl * w, dim=1, keepdim=False)
        # elif slice_att == 'entropy':

        # prob_t3 = torch.sigmoid(cl)
        # prob_t2 = 1.0-prob_t3
        # probs = torch.cat([prob_t3, prob_t2], dim = 2)
        # # n(batch) * d(depth=7)
        # confidence_score = Categorical(probs = probs).entropy()
        # F.relu((cl).tropysign())
...
class Attention_custom_3d(nn.Module):
    def __init__(self, resnet_type=34, encoder_dim_type='2d', att_type=[False]*4, stride_D1=[1, 2, 2, 2],
        ...
        self.encoder = encoder(resnet_type=resnet_type, encoder_dim_type=encoder_dim_type, att_type=att_type,
                               stride_D1=stride_D1, num_classes=num_classes)
