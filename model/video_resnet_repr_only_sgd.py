...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, stem, block_num=[3, 4, 6, 3], image_channel=1, stem_dimension='3d',
                 ...
                 zero_init_residual=False, if_framewise=False, loss = None, loss_param={}):
        ...
        # self.register_parameter(name='center', param=self.loss['category'].center)
        ...
        self.fc = nn.Linear(starting_feature_num * 8 * block.expansion, num_classes)
    ...
    def forward(self, data, if_train=False):
        ...
        # print(f'<model forward> x: {x.type()}')
        # Flatten the layer to fc
        # feat = x.permute(1, 0, 2).flatten(1)
        feat = x.permute(0, 2, 1)
        # print(f'<model forward> feat size: {feat.size()}')
        # print(f'<model forward> feat : {feat.type()}')
        # print(f'<model forward> label: {data["category"]}')
        x = self.fc(feat).squeeze(2)

        temp_idx = torch.nonzero(data['repr_num'].cuda(x.get_device()), as_tuple=True)
        # temp_idx_flat = torch.nonzero(data['repr_num'].flatten(0).cuda(x.get_device())).squeeze(1)
        # feat = feat[:, temp_idx_flat].permute(0,1)
        x_serial = x[ temp_idx ]
        # N -> N*1 -< N*7 -> repr# (>=N)
        # data['category'] = (data['category'].unsqueeze(1).repeat(1, data['repr_num'].size(1)))[ temp_idx ]

        out = {'category': x_serial, 'repr_num': temp_idx}

        if self.return_att_map:
            out.update(self.att_collector())
        if if_train:
            data['repr_num']=data['repr_num'].cuda(x.get_device())

            out.update({'loss': self.loss(x, data['repr_num'], feat, data['category'] )})
            # out.update({'loss': self.loss['category'](x, data['category'])})
            # self.loss['category'].update_center(x, data['repr_num'], feat, data['category'])

        # data['category'] = (data['category'].unsqueeze(1).repeat(1, data['repr_num'].size(1)))[temp_idx]

        return out, (data['category'].unsqueeze(1).repeat(1, data['repr_num'].size(1)))[temp_idx]
